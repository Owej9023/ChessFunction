...
timepermove <- list()
val<-1
#the value below is for testing, theoretically affter running the entire code it should work properlly
#set_value<-"01:00.0"
set_value1<-set_value
# Loop through each value in the 'result_list_times'
for (value in result_list_times) {
  ...
  set_value<-as.character(str_extract_all(as.character(result_list_times[[val]][1]),'\\d+\\d+:\\d+(?:\\.\\d+|)'))
  set_value<-as.POSIXct(set_value, format = "%M:%OS")
  #set_value1<-as.character(str_extract_all(as.character(result_list_times[[val]][1]),'\\d+\\d+:\\d+(?:\\.\\d+|)'))
  #set_value1<-as.POSIXct(set_value1, format = "%M:%OS")
  set_value1<-set_value


  # Loop through each match
  for (val in 1:length(matches)) {
    # Extract the matched string according to move color
    if(val %% 2 == 0){
  ...
  # Append the interior list to the main 'timepermove' list
  timepermove <- c(timepermove, list(timepermoveinterior))
}
#issues revolve around setvalue probably
```
...
ggplot(data = zzz) + 
  ...
  xlim(0,10)
#bins = 100 because there are 100 tenths in 10 seconds
```

```{r}
# Generating dummy data
set.seed(123)
zzztest <- data.frame(V1 = sample(1:2, 4, replace = TRUE))

# Creating the histogram with percentages
library(ggplot2)
library(scales)
