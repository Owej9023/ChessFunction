...
combined_df <- combined_df %>% filter(grepl("Bullet", time_class, ignore.case = TRUE))
...
```{r}
#this code gets the time of each game and the incriment for each game
for (value in combined_df$time_control){
  ...
  if (length(split_strings) == 2) {
    # If there are two values, set selectedIncriment to the second value
    incriment<- TRUE
    selectedIncriment <- as.numeric(split_strings[2])
    #print(selectedIncriment)
  } else {
    incriment <- FALSE
    # If there is only one value, use a default incriment
    selectedIncriment<-0
  }
  ...
  #print(set_value)
  #rint(selectedIncriment)
}

  #I Ignore daily  games for the analysis of time spent per move as the amount of time they acutally have and the time spent on the game are not the same. 
  #Example: the vast majority of players with a 24 hour timer per move will not spend up to 24 hours on that move. Therefore I do not find it significant to look at time spent per move on daily games.


# This code Gets all the indiviudal time stamps for each move made in game
...
#This code transforms the above code into useablee formats
...
#create a function that gets the amount of time spent per move
...
for (value in result_list_times) {
  timepermoveinterior <- list()  # Initialize within the outer loop
  matches <- str_extract_all(value, '\\d+\\d+:\\d+(?:\\.\\d+|)')
  
  for (val in 1:length(matches)) {
    m2 <- as.character(matches[val][1])
...
blackcount <- 0
Value1<-as.numeric(timepermove[[1]][2])
Value2<-as.numeric(timepermove[[1]][1])


for (i in 1:length(timepermove[1:2])) {
  individualList <- timepermove[[i]]
  
  whitetime <- list()
  blacktime <- list()
  for (value in individualList) {
    count <- count + 1
    if (count %% 2 == 0) {
      # get black move
      if (value != as.numeric(timepermove[[i]][2])){
        NewTotal<-(value-Value1)
        print(NewTotal)
        if (NewTotal <= -0.01){
          NewTotal<-as.numeric(timepermove[[i]][2])
        }
        Value1<-value
        #print(Value1)
        blacktime <- c(blacktime, NewTotal)
        #print(value)
      }
      ...
      # get white move
      if (value != as.numeric(timepermove[[i]][1])){
        NewTotal<-value-Value2
        if (NewTotal <= -0.01){
          NewTotal<-as.numeric(timepermove[[i]][1])
        }
        Value1<-value
        whitetime <- c(whitetime, NewTotal)
        #print(value)
      }
      blackcount <- blackcount + 1
    }
...
#this is somewhat working. It needs some more adjustments before it works perfectly. Aka still a bag of bugs and errors
```
...
storedTotal<-0
Value1<-as.numeric(blacktimepermove[1])
Value2<-as.numeric(whitetimepermove[1])
blacktimepermove2<-list()
whitetimepermove2<-list()

for (value in blacktimepermove){
  if (value != as.numeric(blacktimepermove[1])){
    NewTotal<-value-Value1
    Value1<-value
    blacktimepermove2<-append(blacktimepermove2,NewTotal)

  }
}

for (value in whitetimepermove){
  if (value != as.numeric(whitetimepermove[1])){
    NewTotal<-value-Value2
    Value2<-value
    whitetimepermove2<-append(whitetimepermove2,NewTotal)


  }
}

#the code above is trying to get the amount of time spent per move
#currently it is trying its best
#need to seperate games so that they are not all just one big list.
#Need to be able to identify specific games
#issues are currently in timepermove and its children as i modified the time per move function and its output
```
